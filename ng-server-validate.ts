...
export class ServerValidateService {
    private watchingFieldNames = [];
    ...
    clearServerError(form, fieldName) {
        ...
        if (fieldNameIndex > -1) this.watchingFieldNames.splice(fieldNameIndex, 1);
    };
    ...
    addError(form, fieldName, validationProperty) {
        if (form[fieldName]) {
...
function serverValidateDirective(serverValidateService): angular.IDirective {
    return {
        restrict: 'A',
        require: 'form',
        link: function ($scope, $elem, $attrs: any, $form) {
...
export default angular.module('server-validate', [])
